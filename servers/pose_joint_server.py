...
class PoseController(NaoqiNode):
    def __init__(self):
        ...
        self.pip = rospy.get_param('/behaviours_server/pepper_ip')
        self.pport = rospy.get_param('/behaviours_server/pepper_port')
    ...
    def handleJointStiffness(self, msg):
        ...
        try:
            ...
            self.pepper_joints = ['HeadYaw', 'HeadPitch','LShoulderPitch', 'LShoulderRoll', 'LElbowYaw', 'LElbowRoll', 'LWristYaw', 'LHand', 'RShoulderPitch', 'RShoulderRoll', 'RElbowYaw', 'RElbowRoll', 'RWristYaw', 'RHand','HipRoll', 'HipPitch', 'KneePitch']
            for items in self.pepper_joints:
                print(items)
                print self.motionProxy.getStiffnesses(items)
        except RuntimeError,e:
            rospy.logerr("Exception caught:\n%s", e)
    ...
    def executeJointTrajectoryAction(self, goal):
        ...
        while running and not self.jointTrajectoryServer.is_preempt_requested() and not rospy.is_shutdown():
            ...
            if task_id is None:
                ...
                task_id = self.motionProxy.post.angleInterpolation(self.new_names, angles, times, (goal.relative==0))
        ...
        jointTrajectoryResult.goal_position.position = self.motionProxy.getAngles(self.new_names, True)
